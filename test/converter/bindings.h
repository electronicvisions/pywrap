...
struct ConverterTest {
	...
	tuple sink(const int_data_type & data);
	tuple sink(const double_data_type & data);
...
tuple ConverterTest::sink(const int_data_type & data)
{
	return make_tuple(int_tag, data.size());
}

tuple ConverterTest::sink(const double_data_type & data) {
