...
from pygccxml.declarations import declaration_matcher_t, matcher_base_t, namespace
from pygccxml.declarations.runtime_errors import declaration_not_found_t, multiple_declarations_found_t

from pyplusplus.decl_wrappers.calldef_wrapper import casting_operator_t


class namespace_contains_matcher_t(declaration_matcher_t):
    """
    ...
    def __call__(self, decl):
        ...
        while not isinstance(p, namespace.namespace_t):
            p = p.parent
            if p is None:
                return False
...
class match_std_container_t(declaration_matcher_t):
...
class match_casting_operator_t(matcher_base_t):
